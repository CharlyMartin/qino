...
describe.each([
  ...
  ".markdown",
  ".json",
] as const)("%s entries", (extension) => {
  test.each([
    "content",
    "transform",
  ])("rejects reserved fields from %s in getters and CLI validation", async (source) => {
...
test.each([
  ...
  ".markdown",
  ".json",
] as const)("body remains user-defined in %s entries", async (extension) => {
  const raw =
    extension == ".json"
...
test.each([
  "_meta",
  "markdown",
])("augmentation cannot overwrite generated %s", async (field) => {
  await fs.writeFile(
...
test.each([
  ...
  ".mdx",
  ".markdown",
] as const)("getters and CLI accept transformed markdown in %s", async (extension) => {
  await fs.writeFile(
  ...
  const item = qino.defineItem({ file: `/entries/hello${extension}`, schema });
  const collection = qino.defineCollection({
    directory: "/entries",
    extension,
    schema,
