...
- **A per-call `transform`** hits the same problem from the other side: the
...
- **Two `createCollection` calls over the same directory** (one plain, one
  with `transform`) don't actually give two interchangeable views of the
...
**Views** fix this by pairing `resolveRelations` and `transform` together,
per named view, at config time — so within a single view there's no
per-call variability left to reconcile, and `transform` can safely run
...
- A view is `{ resolveRelations?: ResolveOption; transform?: EntryTransform<...> }`.
  Both keys are independently optional; a missing `resolveRelations`
  defaults to `true` and a missing `transform` means no derived fields —
...
- Views **do not inherit from each other**, and named views do **not**
  ...
  same baseline defaults (`resolveRelations: true`, no transform), not to
...
- `resolveRelations` is no longer overridable at the getter call site once
  ...
  config-time-fixed `{ resolveRelations, transform }` pair.
- Ordering, per view: if the view's `resolveRelations` is truthy, relations
  are resolved first and `transform` receives the resolved entry. If
  `resolveRelations` is `false`, `transform` receives the raw (unresolved)
...
- Nested relation resolution is unaffected by any of this. When an entry is
  ...
  and transform never apply. An embedded `author` never carries
...
- The existing (already-implemented) `GetterOptions<R>` lets
  ...
  `transform`. Introducing `views` should retire that per-call override in
  favor of view selection — keeping both would let a caller request a
  view's `transform` while separately forcing `resolveRelations: false`,
  breaking the exact type assumption (transform typed against the resolved
  shape) that views exist to make safe. Confirm this is an intentional
  breaking change to the just-merged transform work, and decide whether
